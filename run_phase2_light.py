...
### Performing 3 design iterations on each input structure
NSTRUCT = 3
...
with open(cmds_filename_des, "w") as file:
    for pdb in good_pmpnn_bb:
        ...
        temperature =list(("0.2", "0.3"))
        distance_redesign_cutoff = list(("8.0", "15.0", "500.0"))
            commands_design.append(f"{PYTHON['ligandMPNN']} {SCRIPT_DIR}/scripts/design/ligMPNN_light_pocket_design.py "
                             f"--pdb {MPNN_DIR}/backbones/{pdb} --nstruct {NSTRUCT} --redesign_d_cutoff {distance_redesign_cutoff} --target_positions {keep_nat}"
                             f" --scoring {SCRIPT_DIR}/scripts/design/scoring/FUN_scoring.py --temperature {temperature} \n" )
...
utils.create_slurm_submit_script(filename=submit_script, name="3.1_design_pocket_ligMPNN", mem="4g", 
                                 N_cores=1, gpu=True, time="40:00:00", array=len(commands_design),
                                 array_commandfile=cmds_filename_des, partition="h100", group=112)
